...
export const RATE_GROUPS = [
  ...
  { key: 'italy-group', label: 'Italy, Bulgaria, Croatia, Estonia, Greece, Lithuania, Finland, Latvia',
  ...
  { key: 'japan', label: 'Tax free Japan',
    flat: [null, 3350, null, 4250, null, 4850, null, 5550, null],
    bands: [1250, 900, 800] },
  { key: 'post-service', label: 'Post service — Denmark, Belgium, France, Austria, Netherlands, Luxemburg, Czech Republic',
...
export function calculate({ destination, weight }) {
  ...
  if (w <= 4.5) {
    // Round up to the nearest listed 0.5 kg bracket.
    ...
    return {
      ...
      basis: `Flat rate (${FLAT_WEIGHTS[idx]} kg)`,
      perKg: null
    };
  }
  ...
  return {
    total: Math.round(w * rate),
    weight: w,
    basis: `Per kg (${BANDS[idx].label})`,
    perKg: rate
  };
}
